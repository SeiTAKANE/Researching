...
#1回目の緊急自体宣言の期間
mobility_jpn_firsts <- mobility_jpn_all  %>% filter(date　>= "2020-04-06" & date　<= "2020-05-25")
mobility_jpn_others <- mobility_jpn_all  %>% filter(date　< "2020-04-06" | date　> "2020-05-25")
...
t.test(mobility_jpn_firsts$retail_and_recreation_percent_change_from_baseline,
       mobility_jpn_others$retail_and_recreation_percent_change_from_baseline,var.equal=T,paired=F) #t = -17.382,95 percent confidence interval:-23.00818 -18.33276 mean: -31.64000 -10.96953   

#食料品、薬局
t.test(mobility_jpn_firsts$grocery_and_pharmacy_percent_change_from_baseline,
       mobility_jpn_others$grocery_and_pharmacy_percent_change_from_baseline,var.equal=T,paired=F) #t = -2.1597 95 percent confidence interval:-3.1184823 -0.1465593 mean:-1.9400000 -0.3074792 

#乗換駅
t.test(mobility_jpn_firsts$transit_stations_percent_change_from_baseline,
       mobility_jpn_others$transit_stations_percent_change_from_baseline,var.equal=T,paired=F)#t = -17.468 confidence interval:-26.02555 -20.76049  mean:-47.08000 -23.68698 

#職場
t.test(mobility_jpn_firsts$workplaces_percent_change_from_baseline,
       mobility_jpn_others$workplaces_percent_change_from_baseline,var.equal=T,paired=F)#t = -7.1408, confidence interval:-17.30078  -9.83152  mean:-26.58000 -13.01385
#住宅
#F検定の結果、等分散でない
t.test(mobility_jpn_firsts$residential_percent_change_from_baseline,
       mobility_jpn_others$residential_percent_change_from_baseline,var.equal=F,paired=F)#t = 11.029 confidence interval:6.302883 9.098225  mean:13.900000  6.199446 
#公園
t.test(mobility_jpn_firsts$parks_percent_change_from_baseline,
       mobility_jpn_others$parks_percent_change_from_baseline,var.equal=T,paired=F)#t = 0.53122 confidence interval: -3.734879  6.500973  mean:  -4.520000 -5.903047 
